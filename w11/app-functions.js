...
const removeResult = (id) => {
  const resultIndex = finalOutputs.findIndex(
    (finalOutputs) => finalOutputs.id === id
  ); // Able to use short hand syntax because only returning a single item
  if (resultIndex > -1) {
...
const runData = (object) => {
  ...
  let decisionObject = determineOutput(
    freeTime,
    object.homeworkDone,
    weekend,
    object.id,
...
const determineOutput = (
  ...
  if (homeworkDone && weekend) {
  ...
  } else if (!weekend && homeworkDone && freeTime >= 0.51) {
    ...
    verdict =
      'No, it is the weekend but you found out the kids have a homework assignment!';
  ...
  if (finalOutputs.length > 0) {
    finalOutputs.forEach((finalOutput) => {
      document
        ...
        .appendChild(generateMessageDom(finalOutput));
    });
  }
};
